...
<head>
    ...
    @endif
    {{-- Components CSS (inlined — no external public/ dependency) --}}
    <style>
    ...
    <style>{!! \App\Services\HtmlSanitizer::sanitizeCSS($__cfg['custom_css'] ?? '') !!}</style>
    @endif
    @if(!empty($__cfg['custom_head_html']))
    {!! \App\Services\HtmlSanitizer::sanitizeHeadHtml($__cfg['custom_head_html'] ?? '') !!}
    @endif
</head>
<body>
    ...
    {{-- Components JS (inlined — no external public/ dependency) --}}
    <script>
