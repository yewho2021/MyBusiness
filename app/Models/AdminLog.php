...
class AdminLog extends Model
{
    ...
    /**
     * Resolve IP geolocation via ipwho.is (free, HTTPS, no key, 10k req/month).
     * Fails silently — geo info is optional.
     *
     * Previously used http://ip-api.com (plaintext HTTP — leaked admin IPs in transit).
     * Switched to HTTPS provider for security.
     */
    public static function resolveIpGeo(string $ip): array
    {
        ...
        try {
            ...
            $json = @file_get_contents("https://ipwho.is/{$ip}", false, $ctx);
            if ($json) {
                ...
                if (($data['success'] ?? false) === true) {
                    return [
                        ...
                        'ip_isp'     => $data['connection']['isp'] ?? null,
                    ];
                }
            }
        } catch (\Exception $e) {
