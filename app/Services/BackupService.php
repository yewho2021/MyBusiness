...
class BackupService
{
    ...
    public function execute(BackupRun $run): void
    {
        ...
        $run->save();
            ...
            // ── Phase 6: Package to ZIP ──────────────────────
            $this->log('info', '────────────────────────────────────────────');
            $this->log('info', '[Phase 6] Packaging backup to ZIP...');

            try {
                $zipPath = $this->zipBackupFolder($backupPath, $folderName);
                $zipSize = filesize($zipPath);
                $relativeZipPath = str_replace(base_path() . '/', '', $zipPath);
                ...
                $this->log('success', 'ZIP created: ' . basename($zipPath) . ' (' . $this->formatBytes($zipSize) . ')');
                $this->log('info', 'Compression ratio: ' . ($totalSize > 0 ? round((1 - $zipSize / $totalSize) * 100) . '%' : 'N/A'));

                // Remove raw folder — ZIP is the source of truth now
                File::deleteDirectory($backupPath);
                $this->log('info', 'Raw folder removed (ZIP only)');
            } catch (\Exception $e) {
                // ZIP failed but backup folder still exists — not fatal
                $this->log('warning', 'ZIP packaging failed: ' . $e->getMessage());
                $this->log('info', 'Backup folder kept as fallback: ' . $folderName);
            }
    ...
    {
        $this->run = $run;
        $tempExtractPath = null; // Track temp dir for cleanup

        try {
            $this->backupBasePath = $this->resolveBasePath($run);
            $backupPath = $run->getBackupPath();

            // ── Handle ZIP backups: extract first ────────────
            $zipPath = $run->getZipPath();
            if ($zipPath && file_exists($zipPath) && !File::isDirectory($backupPath)) {
                $run->update(['status' => BackupRun::STATUS_RESTORING, 'started_at' => now()]);

                $this->log('info', '════════════════════════════════════════════');
                $this->log('info', 'ZIP backup detected — extracting first...');

                $tempExtractPath = $this->extractZipForRestore($zipPath);
                $backupPath = $tempExtractPath; // Point restore at extracted folder

                $this->log('success', 'Extraction complete. Proceeding with restore.');
                $this->log('info', '════════════════════════════════════════════');
            } else {
                $run->update(['status' => BackupRun::STATUS_RESTORING, 'started_at' => now()]);
            }
            ...
            // Clean up temp extracted folder if we were restoring from ZIP
            if ($tempExtractPath && File::isDirectory($tempExtractPath)) {
        ...
        // Clean up temp extracted folder on success too
        if ($tempExtractPath && File::isDirectory($tempExtractPath)) {
            File::deleteDirectory($tempExtractPath);
            $this->log('info', 'Temp extraction folder cleaned up');
        }
    }
    ...
    protected function collectFiles(array $includePaths, array $excludePaths, array $excludeExtensions = []): array
    {
        ...
        $defaultExcludes = [
            'vendor', 'node_modules', 'backup', '.git',
    ...
    protected function isExcluded(string $filePath, array $excludePaths, string $basePath): bool
    {
        ...
        foreach ($excludePaths as $exclude) {
            ...
            // Handle absolute exclude paths (e.g. /home/user/public_html/tests)
            if (str_starts_with($exclude, 'home/') || str_contains($exclude, '/public_html/')) {
                // Convert absolute exclude to relative by removing basePath prefix
                $absExclude = '/' . ltrim($exclude, '/');
                $relExclude = str_replace($basePath . '/', '', $absExclude);
                if (str_starts_with($relativePath, $relExclude . '/') || $relativePath === $relExclude) {
                    return true;
                }
            }
    ...
    /**
     * Package a backup folder into a ZIP file.
     * Returns the full path to the created ZIP.
     */
    protected function zipBackupFolder(string $folderPath, string $folderName): string
    {
        $zipPath = dirname($folderPath) . '/' . $folderName . '.zip';
        $zip = new \ZipArchive();
        ...
        foreach ($iter as $file) {
            if ($file->isFile()) {
                // Relative path inside ZIP: files/app/Models/Admin.php, database.sql, etc.
        ...
        $zip->close();
        $this->log('info', "  Packed {$fileCount} files into ZIP");

        // Verify ZIP is valid
        $verify = new \ZipArchive();
        if ($verify->open($zipPath) !== true) {
            @unlink($zipPath);
            throw new \Exception("ZIP verification failed — file may be corrupt");
        }
        $verifyCount = $verify->numFiles;
        $verify->close();

        if ($verifyCount !== $fileCount) {
            $this->log('warning', "  ZIP file count mismatch: expected {$fileCount}, got {$verifyCount}");
        }
    ...
    /**
     * Extract a ZIP backup to a temporary folder for restore.
     * Returns the path to the extracted folder.
     */
    protected function extractZipForRestore(string $zipPath): string
    {
        $extractPath = sys_get_temp_dir() . '/backup_restore_' . uniqid();

        $zip = new \ZipArchive();
        if ($zip->open($zipPath) !== true) {
            throw new \Exception("Cannot open backup ZIP: {$zipPath}");
        }

        if (!File::makeDirectory($extractPath, 0755, true)) {
            $zip->close();
            throw new \Exception("Cannot create temp directory for extraction");
        }

        $zip->extractTo($extractPath);
        $this->log('info', "  Extracted {$zip->numFiles} files from ZIP");
        $zip->close();
