                    ...
                    <tr>
                        ...
                        <th class="sortable-th" onclick="DatabaseManager.sortTable('{{ $table }}', '{{ $col }}', '{{ $nextDir }}')" title="Sort by {{ $col }}">
                            <span style="display:inline-flex;align-items:center;gap:4px;cursor:pointer;user-select:none">
                                {{ $col }}
                                @if($isSorted)
                                    <i class="fas fa-sort-{{ $sortDir === 'ASC' ? 'up' : 'down' }}" style="color:#2563eb;font-size:12px"></i>
                                @else
                                    <i class="fas fa-sort" style="color:#d1d5db;font-size:11px"></i>
                                @endif
                            </span>
                        </th>
                        @endforeach
        ...
        <div class="pager" data-table="{{ $table }}">
            <div style="display:flex;align-items:center;gap:8px">
                ...
                @if(!empty($sortCol))
                    ...
                    <span style="font-size:13px;color:#2563eb;background:#eff6ff;padding:2px 8px;border-radius:4px;display:inline-flex;align-items:center;gap:4px">
