...
.breadcrumb{font-size:13px;color:#64748b;margin-bottom:4px}.breadcrumb a{color:#2563eb;text-decoration:none}
...
.badge-completed{background:#dcfce7;color:#166534}.badge-running{background:#dbeafe;color:#1e40af}.badge-pending{background:#fef3c7;color:#92400e}.badge-failed{background:#dbeafe;color:#991b1b}.badge-restored{background:#f3e8ff;color:#6b21a8}.badge-restoring{background:#e0f2fe;color:#075985}
.progress-bar{background:#e2e8f0;border-radius:10px;height:28px;overflow:hidden;position:relative}
.progress-fill{height:100%;border-radius:10px;transition:width .8s ease;background:#dc2626;min-width:40px}
...
.error-box{background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:12px 16px;color:#991b1b;font-size:13px;margin:0 20px 20px}
...
.status-banner.failed{background:#eff6ff;color:#991b1b}
...
.duration-live{font-family:'Courier New',monospace;font-size:16px;font-weight:700;color:#2563eb;letter-spacing:1px}
</style>
@endpush
@section('content')
...
<button class="btn-outline" style="color:#ef4444;border-color:#bfdbfe"><i class="fas fa-trash"></i> Clear Logs</button>
</form>
@endif
</div>
...
<a href="{{ $logs->url(1) }}" style="color:#f87171;text-decoration:none;padding:4px 8px;border:1px solid #334155;border-radius:4px;font-size:12px">First</a>
<a href="{{ $logs->previousPageUrl() }}" style="color:#f87171;text-decoration:none;padding:4px 8px;border:1px solid #334155;border-radius:4px;font-size:12px">&laquo;</a>
@endif
@php $from=max(1,$logs->currentPage()-3);$to=min($logs->lastPage(),$logs->currentPage()+3); @endphp
@for($i=$from;$i<=$to;$i++)
@if($i==$logs->currentPage())<span style="background:#dc2626;color:#fff;padding:4px 8px;border-radius:4px;font-size:12px">{{ $i }}</span>
@else<a href="{{ $logs->url($i) }}" style="color:#f87171;text-decoration:none;padding:4px 8px;border:1px solid #334155;border-radius:4px;font-size:12px">{{ $i }}</a>@endif
@endfor
@if($logs->hasMorePages())
<a href="{{ $logs->nextPageUrl() }}" style="color:#f87171;text-decoration:none;padding:4px 8px;border:1px solid #334155;border-radius:4px;font-size:12px">&raquo;</a>
<a href="{{ $logs->url($logs->lastPage()) }}" style="color:#f87171;text-decoration:none;padding:4px 8px;border:1px solid #334155;border-radius:4px;font-size:12px">Last</a>
@endif
</div>
</div>
@endif
