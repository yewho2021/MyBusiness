...
class DatabaseController extends Controller
{
     ...
     * User queries (including USE statements) run here so they
     * never pollute the app's main mysql connection.
     */
    ...
    public function viewTable($table)
    {
        ...
        $rows = $conn->select("SELECT * FROM `{$table}` LIMIT {$perPage} OFFSET {$offset}");
        ...
        if (request()->ajax()) {
            return view('admin.pages.database.partials.table_content', compact('table', 'columns', 'indexes', 'rows', 'totalRows', 'page', 'perPage', 'totalPages', 'tableInfo', 'createSql', 'offset', 'pkColumn'));
        }
        return view('admin.pages.database.table', compact('table', 'columns', 'indexes', 'rows', 'totalRows', 'page', 'perPage', 'totalPages', 'tableInfo', 'createSql', 'offset', 'pkColumn'));
    }
        ...
        if ($request->isMethod('post') && !empty($sql)) {
            ...
            foreach (['DROP DATABASE', 'GRANT ', 'REVOKE ', 'CREATE USER', 'DROP USER'] as $b) {
                if (stripos($sql, $b) !== false) {
                    ...
                    return view('admin.pages.database.query', compact('sql', 'results', 'error', 'affectedRows', 'executionTime', 'columns', 'bookmarks', 'recentHistory', 'tableList', 'dbName', 'totalSize'));
                }
            }
